...
SELECTED_BORDER = "#2563EB"     # blue highlight for the selected card
...
class SearchMixin:
    ...
    def _page_search(self, p):
        ...
        # Ctrl+F focuses the medicine field
    ...
    # ------------------------------------------------------------------ #
    #  Snapshot tags
    #  customtkinter forbids 'font' in CTkTextbox.tag_config because it
    #  would break scaling. We try the internal tkinter.Text so we can
    #  still use bold headers, and fall back to color-only if the private
    #  attribute ever disappears.
    # ------------------------------------------------------------------ #
    def _style_snapshot_tags(self):
        inner = getattr(self.snapshot, "_textbox", None)
        if inner is not None:
            try:
                inner.tag_config("title",   font=("Consolas", 11, "bold"))
                inner.tag_config("section", font=("Consolas", 10, "bold"))
                inner.tag_config("muted",   foreground=MUTED)
                return
            except Exception:
                pass  # fall through to color-only

        # Fallback: color-only tags (public API)
        try:
            self.snapshot.tag_config("title",   foreground="#111827")
            self.snapshot.tag_config("section", foreground="#374151")
            self.snapshot.tag_config("muted",   foreground=MUTED)
        except Exception:
            # If even color fails, tags are simply unused. Harmless.
            pass
    ...
    def _refresh_search(self):
        ...
        # Reset UI state before hitting the DB so an exception leaves us
        # in a consistent (empty) state.
        self._clear_results()
        ...
        count = len(rows)
        suffix = f" (showing first {MAX_RESULTS})" if count == MAX_RESULTS else ""
